...
namespace Kraken {
namespace Private {
OpenOrders::OpenOrders(struct kraken_api **kr_api) {
	...
	for (auto& m : a.GetObject()) //Iterate through the orders
	{
		...
		this->orders.push_back({txid,refid,userref,status,opentm,starttm,expiretm,pair,type,ordertype,price,price2,
			leverage,order,close,vol,vol_exec,cost,fee,averagePrice,stopprice,limitprice});
	}
}
std::vector<OpenOrders::Order> OpenOrders::getOrders()
{
	return this->orders;
}

void OpenOrders::printOrders ()
{
	if(orders.empty())
	...
	{
		printf("%s\n",this->orders[i].txid);
		printf("\t%s, %f, %s, %f, %f, %f, %s, %s, %s, %f, %f, %s, %s, %s, %f, %f, %f, %f, %f, %f, %f\n",
			this->orders[i].refid,this->orders[i].userref,this->orders[i].status, this->orders[i].opentm,this->orders[i].starttm,
				this->orders[i].expiretm,this->orders[i].pair,this->orders[i].type,this->orders[i].ordertype,this->orders[i].price,
				this->orders[i].price2,this->orders[i].leverage, this->orders[i].order,this->orders[i].close,this->orders[i].vol,
				this->orders[i].vol_exec,this->orders[i].cost,this->orders[i].fee,this->orders[i].averagePrice,
				this->orders[i].stopprice,this->orders[i].limitprice);
	}
