#include "OrdersInfo.h"

namespace Kraken {
namespace Private {
OrdersInfo::OrdersInfo(struct kraken_api **kr_api, const char* txid) {
	(*kr_api)->priv_func->query_order_info(kr_api,txid);
	...
	{
		const char* _txid = m.name.GetString(); //transaction id
		...
		this->orders.push_back({refid,userref,status,reason,opentm,closetm,starttm,expiretm,pair,type,ordertype,price,price2,
			leverage,order,close,vol,vol_exec,cost,fee,averagePrice,stopprice,limitprice});
	}
}
std::vector<OrdersInfo::Order> OrdersInfo::getOrderInfo()
{
	return this->orders;
}

void OrdersInfo::printOrderInfo ()
{
	if(orders.empty())
